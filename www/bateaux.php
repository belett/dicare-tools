...
define('PAGE_TITLE', 'Bateaux');
require '../inc/header.inc.php';

echo '<h1>Bateaux</h1>
<form method="post" action="bateaux.php">
<p>Cet outil génère, à partir des données Wikidata, une liste de bateaux (instances, ou instances sous-classe, de <a href="https://www.wikidata.org/wiki/Q1229765">Q1229765</a>) selon différents critères. Au moins un critère doit être utilisé.</p>
<p><label for="id">Identifiant Wikidata d\'une fête maritime</label> (exemple : Q20971895 pour <em>Brest 2016</em>) :<br /><input type="text" id="id" name="id" value="'.htmlentities(page::getParameter('id')).'" /></p>
<p>
...
';
    ...
    echo '<h2>'.count($items->results->bindings).' résultats [<a href="?id='.urlencode(page::getParameter('id')).'&amp;bip='.($bip ? 'true' : 'false').'&amp;palissy='.($palissy ? 'true' : 'false').'">Permalien</a>]</h2>';
    if (count($items->results->bindings) === 0) {
        echo '<p>Aucun résultat.</p>';
    }
    else {
        echo '<table class="ships sortable">
<thead><tr><th>Wikidata</th><th>Libellé</th><th><abbr title="International Maritime Organization">IMO</abbr></th><th><abbr title="Maritime Mobile Service Identity">MMSI</abbr></th><th><abbr title="Bateau d\'intérêt patrimonial">BIP</abbr></th><th><abbr title="Palissy">Pal.</abbr></th><th><abbr title="Immatriculation">Imm.</abbr></th><th><abbr title="Fanion">Fan.</abbr></th><th><abbr title="Radio">Rad.</a></th><th>Commons</th><th>Site web</th></tr>
<tbody>'."\n";
        foreach ($items->results->bindings as $item) {
            echo '<tr>';
            
            $itemId = substr($item->item->value, 32);
            echo '<td><a href="https://www.wikidata.org/wiki/Q'.$itemId.'">'.$itemId.'</a></td>';
            ...
            if (empty($item->commons->value)) {
                ...
                foreach ($commons as &$val) {
                    $val = substr($val, 46);
                    $val = '<a href="https://commons.wikimedia.org/wiki/Category:'.str_replace(' ', '_', urldecode($val)).'">'.htmlentities(urldecode($val)).'</a>';
                }
