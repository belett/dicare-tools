...
$query = 'SELECT ?departement ?insee ?label (COUNT(*) AS ?count)
WHERE {
    ?item wdt:P31 wd:Q5 .
...
GROUP BY ?departement ?insee ?label
ORDER BY ?insee';
$items = sparql::query($query, 3600);
foreach ($items->results->bindings as $item) {
    $departements[$item->insee->value]['departement'] = substr($item->departement->value, 31);
...
foreach ($departements as $insee => $value) {
    ...
    echo '<tr><td class="label"><strong>'.htmlentities($insee).'</strong> <a href="nom-de-famille-suggestion.php?id='.htmlentities($value['departement']).'">'.htmlentities($value['label']).'</a></td><td>'.htmlentities(display::formatInt($value['total'])).'</td><td>'.htmlentities(display::formatInt($value['lastname'])).'</td><td>'.number_format($ratio_lastname, 1, ',', ' ').'</td><td class="ratio"><img src="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mP8z8DwHwAFBQIAX8jx0gAAAABJRU5ErkJggg==" style="height: 12px; width:'.round($ratio_lastname * 2).'px;" /></td><td>'.htmlentities(display::formatInt($value['firstname'])).'</td><td>'.number_format($ratio_firstname, 1, ',', ' ').'</td></tr>'."\n";
}
