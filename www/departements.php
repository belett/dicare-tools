...
foreach ($departements as $insee => &$value) {
    ...
    echo '<tr><td class="label"><strong>'.htmlentities($insee).'</strong> <a href="nom-de-famille-suggestion.php?id='.htmlentities($value['qid']).'">'.htmlentities($value['label']).'</a></td><td>'.htmlentities(display::formatInt($value['total'])).'</td><td>'.htmlentities(display::formatInt($value['lastname'])).'</td><td>'.(($value['total'] >= 1) ? number_format($ratio_lastname, 1, ',', ' ') : '').'</td><td class="ratio"><img src="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mP8z8DwHwAFBQIAX8jx0gAAAABJRU5ErkJggg==" style="height: 12px; width:'.round($ratio_lastname * 2).'px;" /></td><td>'.htmlentities(display::formatInt($value['firstname'])).'</td><td>'.(($value['total'] >= 1) ? number_format($ratio_firstname, 1, ',', ' ') : '').'</td></tr>'."\n";
}
